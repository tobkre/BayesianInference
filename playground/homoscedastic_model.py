...
class test_model(nn.Module):
    def __init__(self, n_hidden, weight_regularizer, sigma_init=1, noise='homoscedastic'):
...
if __name__=='__main__':
    import pylab
    N = 50
    t_par = training_parameters(lr=0.10, n_epochs=5000, verbose=False)
    ...
    res_mu = np.empty((2, len(y_tst)))
    res_sig = np.empty((2, len(y_tst)))
    
    for i in range(2):
        model = test_model(100, weight_regularizer=1.95e-2, noise='heteroscedastic')
...
#        pylab.fill_between(x_train.squeeze(), lc, uc, alpha=0.5)
    ...
    var = 1/2*np.sum(res_mu**2, 0)-(1/2*np.sum(res_mu, 0))**2+1/2*np.sum(res_sig**2, 0)
